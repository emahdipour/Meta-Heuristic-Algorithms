...
dim=30;
memeplex=20;
xl=-5; %-5.12; %-500; %-5; %-10; %-2.048; % -5; %-100; %-32.768; %-600; %
xh=10; %5.12; %500; %10; %10; %2.048; %10; %100; %32.768; %600; %
    ...
    while(k<memeplex)
        while(u<iteration)
           ...
           % disp('*** memeplex ***');
            ...
            c1=2;
            for i =1:dim
                TeV(i)=worstFrog(:,:,i,2)+r1*c1*(bestFrog(:,:,i,1)-worstFrog(:,:,i,1));
                if(TeV(i)<=maxSpeed)
                    TeX(i)=worstFrog(:,:,i,1)+TeV(i);
                else
                    TeV(i)=maxSpeed;
                    TeX(i)=worstFrog(:,:,i,1)+TeV(i);
                    if(TeX(i)>high) 
            ...
            if(FitNew<fitworstFrog)
                ...
                r1=rand() ;               
                for i =1:dim
                    TeV(i)=worstFrog(:,:,i,2)+r1*c1*(GBestFrog(1,i,1)-worstFrog(:,:,i,1));
                    if(TeV(i)<=maxSpeed)
                        TeX(i)=worstFrog(:,:,i,1)+TeV(i);
                    else
                        TeV(i)=maxSpeed;
